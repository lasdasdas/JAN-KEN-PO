...
# Parsing
parser = argparse.ArgumentParser(description="You can enable serial with -s")
parser.add_argument('-s', action='store_true', help='Enable serial')
...
while(True):
    ...
    if args.d:
        ...
        cv2.putText(original, (
                    "r: "+str(int(round(pred[0][0]*100))).zfill(2) +
                    " p: "+str(int(round(pred[0][1]*100))).zfill(2) +
                    " s: "+str(int(round(pred[0][2]*100))).zfill(2)),
